...
Designed for direct passthrough to [VBA-WinOCR](https://github.com/rafael-yml/VBA-WinOCR)'s `OCR_BytesToText` function, or any other in-memory consumer.
...
For Each img In oImages
    ...
    aBytes = img
    Dim sText As String
    sText = OCR_BytesToText(aBytes)   ' VBA-WinOCR
Next img
```
...
**`Word_GetImages` touches disk transiently.** `Chart.Export` has no stream-based API -- each image is written to `%TEMP%` and immediately read back and deleted. No permanent files are left by the function under normal operation.
